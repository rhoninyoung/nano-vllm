...
class ModelRunner:
    ...
    def allocate_kv_cache(self):
        ...
        if self.device == "cpu":
            avail_bytes = self._get_available_memory_bytes()
            config.num_kvcache_blocks = int(avail_bytes * config.gpu_memory_utilization * 0.5) // block_bytes
        else:
        ...
        assert config.num_kvcache_blocks > 0, f"Not enough memory for KV cache (computed {config.num_kvcache_blocks} blocks)"
    ...
    def prepare_block_tables(self, seqs: list[Sequence]):
        ...
        return self._to_device(torch.tensor(block_tables, dtype=torch.int32))
    ...
    def prepare_prefill(self, seqs: list[Sequence]):
        ...
        slot_mapping = self._to_device(torch.tensor(slot_mapping, dtype=torch.int32))
    ...
    def prepare_sample(self, seqs: list[Sequence]):
        ...
        return self._to_device(torch.tensor(temperatures, dtype=torch.float32))
